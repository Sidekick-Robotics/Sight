<div align="center">
<img src="./Ui/SideKick_Logo.png" style="width:20%;">
</div>
<div align="center"><h1>Sight</h1></div>
<div align="center">Making software development easier for embedded systems.</div>
<br>
<p align="center">
  <img src="https://github.com/Sidekick-Robotics/Sight/actions/workflows/pylint.yml/badge.svg" alt="PyLint">&nbsp;&nbsp;&nbsp;<img src="https://github.com/Sidekick-Robotics/Sight/actions/workflows/integration.yml/badge.svg" alt="integration">
</p>
<div align="center"><h2>Contents</h2></div>

1. <a href="#installation">Installation</a>
    1. <a href="#windows">Windows</a>
    2. <a href="#development">Development</a>
2. <a href="#instructions">Instructions</a>
    1. <a href="#home-screen">Home screen</a>
        1. <a href="#terminal">Terminal</a>
        2. <a href="#graphs">Graphs</a>
        3. <a href="#recording">Recording</a>
        4. <a href="#compile-and-upload">Compile and Upload</a>
    2. <a href="#menu">Menu</a>
        1. <a href="#projects">Projects</a>
        2. <a href="#saves">Saves</a>
        3. <a href="#library-manager">Library Manager</a>
    3. <a href="#device">Device</a>
        1. <a href="#testing">Testing</a>
        2. <a href="#board-settings">Board Settings</a>
        3. <a href="#boards-manager">Boards Manager</a>
    4. <a href="#arduino-cli">Arduino CLI</a>
3. <a href="#reporting-bugs">Reporting Bugs</a>
5. <a href="#contributing">Contributing</a>

<div align="center"><h2 id="installation">Installation</h2></div>
<h3 id="windows">Windows</h3>

<h3 id="development">Development</h3>

<div align="center"><h2 id="instructions">Instructions</h2></div>
<h3 id="home-screen">Home screen</h3>

<h4 id="terminal">Terminal</h4>

<h4 id="graphs">Graphs</h4>

<h4 id="recording">Recording</h4>

<h4 id="compile-and-upload">Compile and Upload</h4>

<h3 id="menu">Menu</h3>

<h4 id="projects">Projects</h4>

<h4 id="saves">Saves</h4>

<h4 id="library-manager">Library Manager</h4>

<h3 id="device">Device</h3>

<h4 id="testing">Testing</h4>

<h4 id="board-settings">Board Settings</h4>

<h4 id="boards-manager">Boards Manager</h4>

<h3 id="arduino-cli">Arduino CLI</h3>

<div align="center"><h2 id="reporting-bugs">Reporting Bugs</h2></div>

<div align="center"><h2 id="contributing">Contributing</h2></div>
